...
// BMPtoCNV converts a BMP file back to the proprietary CNV format
func BMPtoCNV(filePath string) ([]byte, error) {
	// Read the image file
	...
	_, ext := fileutil.GetLowerBaseAndExt(filePath)
	if ext != ".bmp" {
		return nil, fmt.Errorf("only BMP files are supported, got: %s", ext)
	}

	// Decode the BMP image
	img, err := identifyImageFormat(filePath, fileData)
	if err != nil {
...
func identifyImageFormat(filePath string, data []byte) (image.Image, error) {
	_, ext := fileutil.GetLowerBaseAndExt(filePath)
